...
#' deviationScores
#'
#' Accessor for raw deviation scores from 
#' \code{\link{chromVARDeviations-class}} object
...
#' @return matrix of raw deviation scores 
#' @author Alicia Schep
#' @examples
#' # Load very small example results from computeDeviations
#' data(mini_dev, package = "chromVAR")
...
#' \code{\link{chromVARDeviations-class}} object
#' @rdname rawDeviations
#' @name rawDeviations
#' @aliases rawDeviations,chromVARDeviations-method
#' @param object chromVARDeviations object
#' @return The rawDeviations and deviations accessors both return matrices.
#' @return matrix of deviation Z-scores
#' @author Alicia Schep
#' @examples
#' # Load very small example results from computeDeviations
#' data(mini_dev, package = "chromVAR")
#' scores <- rawDeviations(mini_dev)
setGeneric("rawDeviations",
           function(object) standardGeneric("rawDeviations"))
...
#' @rdname rawDeviations
#' @export
setMethod("rawDeviations", c(object = "chromVARDeviations"),
          function(object) {
            assays(object)$raw_deviations
          })
...
setMethod("rbind", "chromVARDeviations",
          function(..., deparse.level=1){
            ...
            rawdev <- do.call(rbind,
                              lapply(inputs, function(x) assays(x)$raw_deviations))
            rd <- do.call(rbind, lapply(inputs,rowData))
            out <- SummarizedExperiment(assays = list(z = z,
                                                      deviations = dev,
                                                      raw_deviations = rawdev),
...
compute_deviations_core <- function(counts_mat,
  ...
  rawdev <- t(vapply(results, function(x) x[["rawdev"]], rep(0, ncol(counts_mat))))

  colnames(z) <- colnames(dev) <- colnames(rawdev) <- sample_names
  ...
  out <- SummarizedExperiment(assays = list(deviations = dev, z = z, raw_deviations = rawdev),
                              colData = colData,
                              rowData = rowData)
...
compute_deviations_single <- function(peak_set,
  ...
  if (intermediate_results) {
    ...
    out <- list(z = z, dev = normdev, matches = tf_count / nrow(counts_mat),
                overlap = bg_overlap, rawdev = observed_deviation)
  }
  return(out)
}
